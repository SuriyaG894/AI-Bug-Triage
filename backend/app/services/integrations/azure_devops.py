...
<tr><td><b>Severity:</b></td><td>{severity}</td></tr>
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code in [200, 201]:
                ...
                if attachments and len(attachments) > 0 and attachments[0] != "No attachments":
                    await self._upload_and_link_attachment(client, work_item_id, attachments, work_item_base_url)
    ...
    async def _upload_and_link_attachment(self, client: httpx.AsyncClient, work_item_id: int, attachments: List[Any], base_url: str = None) -> None:
...
def create_azure_client(credentials: str = "", project: str = None) -> Optional[AzureDevOpsClient]:
